...
public class EntityFactory {
    ...
    public Player buildPlayer(Position pos) {
        ...
        return new Player(pos, playerHealth, playerAttack, this, game);
    }
    ...
    public Sceptre buildSceptre() {
        int mindControlDuration = config.optInt("mind_control_duration", 10);
        return new Sceptre(mindControlDuration);
    }

    public MidnightArmour buildMidnightArmour() {
        double attackBonus = config.optDouble("midnight_armour_attack_bonus", 5.0);
        double defenceBonus = config.optDouble("midnight_armour_defence_bonus", 5.0);
        return new MidnightArmour(attackBonus, defenceBonus);
    }
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        case "red_slime":
            return buildRedSlime(pos);
        case "green_slime":
            return buildGreenSlime(pos);
        case "blue_slime":
            return buildBlueSlime(pos);
        case "sun_stone":
    ...
    public RedSlime buildRedSlime(Position pos) {
        double damage = config.optDouble("red_slime_attack", RedSlime.DEFAULT_DAMAGE);
        return new RedSlime(pos, damage);
    }

    public GreenSlime buildGreenSlime(Position pos) {
        double damage = config.optDouble("green_slime_attack", GreenSlime.DEFAULT_DAMAGE);
        return new GreenSlime(pos, damage);
    }

    public BlueSlime buildBlueSlime(Position pos) {
        double damage = config.optDouble("blue_slime_attack", BlueSlime.DEFAULT_DAMAGE);
        return new BlueSlime(pos, damage);
    }
